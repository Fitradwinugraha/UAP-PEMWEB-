...
<body>
    ...
    <div class="main-content">
        <header>
            <div class="menu-toggle">Data Kucing</div>
        </header>
        <div class="content">
            <div class="top-bar">
                ...
                <button class="add-data" onclick="location.href='tambahkucing.php'">+ Tambah Data</button>
            </div>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    // Query SQL untuk mengambil data kucing dari database berdasarkan kata kunci pencarian
                    if (isset($_GET['keyword']) && !empty($_GET['keyword'])) {
                        $keyword = mysqli_real_escape_string($mysqli, $_GET['keyword']);
                        $query = "SELECT * FROM tabel_kucing WHERE nama_kucing LIKE '%$keyword%' OR jenis_kucing LIKE '%$keyword%' OR nama_pemilik LIKE '%$keyword%'";
                    } else {
                        // Query SQL untuk mengambil semua data kucing jika tidak ada kata kunci pencarian
                        $query = "SELECT * FROM tabel_kucing";
                    }

                    // Eksekusi kueri
                    $result = mysqli_query($mysqli, $query);

                    // Perulangan untuk menampilkan data kucing dalam tabel
                    $no = 1;
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo "<tr>";
                        echo "<td>" . $no++ . "</td>";
                        echo "<td>" . $row['nama_kucing'] . "</td>";
                        echo "<td>" . $row['jenis_kucing'] . "</td>";
