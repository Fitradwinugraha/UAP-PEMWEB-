...
<head>
    ...
    <title>Meow Hotel - Pemilik</title>
...
<body>
    ...
    <div class="main-content">
        <header>
            <div class="menu-toggle">Pemilik</div>
        </header>
        <div class="content">
            <div class="top-bar">
                ...
                <button class="add-data" onclick="location.href='tambahpemilik.php'">+ Tambah Data</button>
            </div>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    // Query SQL untuk mengambil data pemilik dari database berdasarkan kata kunci pencarian
                    if (isset($_GET['keyword']) && !empty($_GET['keyword'])) {
                        $keyword = mysqli_real_escape_string($mysqli, $_GET['keyword']);
                        $query = "SELECT * FROM pemilik WHERE nama_pemilik LIKE '%$keyword%' OR no_telepon LIKE '%$keyword%' OR alamat LIKE '%$keyword%'";
                    } else {
                        // Query SQL untuk mengambil semua data pemilik jika tidak ada kata kunci pencarian
                    ...
                    // Perulangan untuk menampilkan data pemilik dalam tabel
                    $no = 1;
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo "<td>";
                        echo "<button class='edit' onclick='editData(" . $row['id_pemilik'] . ")'><i class='fas fa-edit'></i></button>";
                        echo "<button class='delete' onclick='deleteData(" . $row['id_pemilik'] . ")'><i class='fas fa-trash-alt'></i></button>";
                        echo "</td>";
                        echo "</tr>";
                    }
                    ?>
    ...
    <script>
        function deleteData(id) {
            if (confirm("Apakah Anda yakin ingin menghapus data ini?")) {
                window.location.href = "hapuspemilik.php?id=" + id;
            }
        }

        function editData(id) {
            window.location.href = "editpemilik.php?id=" + id;
        }
    </script>
</body>
</html>
